    ...
    data: (invoices) {
      return expensesAsync.when(
        ...
        data: (expenses) {
          ...
          final ytdInvoices = invoices.where(
            (i) =>
                i.dateIssued.isAfter(
          ...
          final ytdExpensesList = expenses.where(
            (e) =>
                e.date.isAfter(startOfYear.subtract(const Duration(days: 1))),
          );

          final revenue = ytdInvoices.fold(
            0.0,
          ...
          // 2. VAT Turnover (LTM)
          final ltmInvoices = invoices.where(
            (i) =>
                i.dateIssued.isAfter(startOfLtm) &&
                i.status != InvoiceStatus.draft &&
                i.status != InvoiceStatus.cancelled,
          );
          final vatTurnover = ltmInvoices.fold(
            0.0,
            (sum, i) => sum + i.totalAmount,
          );
